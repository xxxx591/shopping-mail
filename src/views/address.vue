        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(val,index) in addressListFilter" :key="index" :class="{'check':checkIndex == index}" @click="checkIndex = index">
                    <dl>
                      <dt>{{val.userName}}</dt>
                      <dd class="address">{{val.streetName}}</dd>
                    ...
                    <div class="addr-opration addr-set-default" v-if="val.isDefault !='1'">
                      <a href="javascript:;" class="addr-set-default-btn"  @click="updatedAddress(val.id,val.isDefault)"><i>设置为默认收货地址</i></a>
                    </div>
                    <div class="addr-opration addr-default"  v-else >默认收货地址</div>
                  </li>
            ...
            <div class="shipping-method-wrap">
              <div class="shipping-method">
                <ul>
                  <li class="check">
                    <div class="name">默认快递</div>
                    <div class="price">免费</div>
                    <div class="shipping-tips">
                      <p>一旦发货,订单应该在2 - 3工作日抵达目的地</p>
                    </div>
                  </li>
                </ul>
              </div>
...
export default {
  ...
  methods: {
    ...
    updatedAddress(addressId, addressDefault) {
      axios
        .post("/api/address/updatedAddress", {
          addressId: addressId
        })
        .then(res => {
          console.log(res);
